...
namespace SugarM.Controllers
{
    public class CarTypeMinorController : BaseController<CarTypeMinor>
    {
        ...
        public CarTypeMinorController(IClientNotification clientNotification, IUserprofileRepository IUserprofileRepository, UserManager<ApplicationUser> userManager)
        {
        ...
        public async Task<IActionResult> SaveCarMinor(CarTypeMinor _CarTypeMinor, string IsEditMode)
        {
            var UserCompCode = GetCurrenCompCode();
            var _UserProfile = await _IUserprofileRepository.GetUserProfile(UserCompCode);
